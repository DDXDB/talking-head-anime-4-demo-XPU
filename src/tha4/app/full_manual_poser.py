...
class MainFrame(wx.Frame):
    ...
    def update_images(self, event: wx.Event):
        ...
        with torch.no_grad():
            start_xpu_event = torch.xpu.Event(enable_timing=True)
            end_xpu_event = torch.xpu.Event(enable_timing=True)
            start_xpu_event.record()
            start_time = time.time()

            output_image = self.poser.pose(self.torch_source_image, pose, output_index)[0].detach().cpu()

            end_time = time.time()
            end_xpu_event.record()
            torch.xpu.synchronize()
            print("xpu time (ms):", start_xpu_event.elapsed_time(end_xpu_event))
